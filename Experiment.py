...
expInfo = {
    "participant": "",
    "sex": ["Male", "Female", "Other", "Prefer not to respond"],
    "age": "",
    "session": "001",
}
...
subj = f"sub-{subj_num}"
subj_out = os.path.join(_thisDir, f"data/{subj}")
...
filename = (
    _thisDir
    + os.sep
    + "data/%s/%s_%s_%s" % (subj, expInfo["participant"], expName, expInfo["date"])
)
...
thisExp = data.ExperimentHandler(
    ...
    extraInfo=expInfo,
    runtimeInfo=None,
    originPath=os.path.join(_thisDir, "cal_abstract.py"),
    savePickle=True,
    saveWideText=True,
...
instruct_text1 = visual.TextStim(
    win=win,
    name="instruct_text1",
    text="default text",
    font="Arial",
...
instruct_text2 = visual.TextStim(
    win=win,
    name="instruct_text2",
    text="Press “enter” or “return” to continue.",
    font="Arial",
...
feedback_text1 = visual.TextStim(
    win=win,
    name="feedback_text1",
    text="default text",
    font="Arial",
...
intructRemind_text1 = visual.TextStim(
    win=win,
    name="intructRemind_text1",
...
instructRemind_text2 = visual.TextStim(
    win=win,
    name="instructRemind_text2",
    text="Press “enter” or “return” to continue.",
    font="Arial",
    pos=(0, -0.35),
    height=0.03,
...
feedback_text2 = visual.TextStim(
    win=win,
    name="feedback_text2",
    text="default text",
    font="Arial",
...
feedback_text3 = visual.TextStim(
    win=win,
    name="feedback_text3",
    text="default text",
    font="Arial",
...
while continueRoutine:
    ...
    if welcome_resp.status == NOT_STARTED and tThisFlip >= 0.0 - frameTolerance:
        ...
        win.callOnFlip(
            welcome_resp.clearEvents, eventType="keyboard"
    ...
    if welcome_resp.status == STARTED and not waitOnFlip:
        ...
        if len(_welcome_resp_allKeys):
            welcome_resp.keys = _welcome_resp_allKeys[
                -1
...
Instruction_loop = data.TrialHandler(
    nReps=1,
    method="sequential",
    extraInfo=expInfo,
    originPath=-1,
...
thisInstruction_loop = Instruction_loop.trialList[
    0
...
for thisInstruction_loop in Instruction_loop:
    ...
    while continueRoutine:
        ...
        if instruct_resp1.status == NOT_STARTED and tThisFlip >= 0.0 - frameTolerance:
            ...
            win.callOnFlip(
                instruct_resp1.clearEvents, eventType="keyboard"
            )  # clear events on next screen flip
        if instruct_resp1.status == STARTED and not waitOnFlip:
            ...
            if len(_instruct_resp1_allKeys):
                instruct_resp1.keys = _instruct_resp1_allKeys[
                    -1
                ].name  # just the last key pressed
...
while continueRoutine:
    ...
    if start_resp1.status == NOT_STARTED and tThisFlip >= 0.0 - frameTolerance:
        ...
        win.callOnFlip(
            start_resp1.clearEvents, eventType="keyboard"
        )  # clear events on next screen flip
    if start_resp1.status == STARTED and not waitOnFlip:
        ...
        if len(_start_resp1_allKeys):
            start_resp1.keys = _start_resp1_allKeys[
                -1
            ].name  # just the last key pressed
...
# set up handler to look after randomisation of conditions etc
Block1_loop = data.TrialHandler(
    nReps=1,
    method="sequential",
    extraInfo=expInfo,
    originPath=-1,
    trialList=data.importConditions(f"blocks/{subj}/Block1.csv"),
    seed=None,
    name="Block1_loop",
)
    ...
    while continueRoutine:
        ...
        if key_resp1.status == STARTED and not waitOnFlip:
            ...
            if len(_key_resp1_allKeys):
                key_resp1.keys = _key_resp1_allKeys[
                    -1
                ].name  # just the last key pressed
                key_resp1.rt = _key_resp1_allKeys[-1].rt
                # was this correct?
                if (key_resp1.keys == str(Block1Corr)) or (
                    key_resp1.keys == Block1Corr
                ):
                    key_resp1.corr = 1
                else:
                    key_resp1.corr = 0
...
while continueRoutine:
    ...
    if break_resp1.status == NOT_STARTED and tThisFlip >= 0.0 - frameTolerance:
        ...
        win.callOnFlip(
            break_resp1.clearEvents, eventType="keyboard"
        )  # clear events on next screen flip
    if break_resp1.status == STARTED and not waitOnFlip:
        ...
        if len(_break_resp1_allKeys):
            break_resp1.keys = _break_resp1_allKeys[
                -1
            ].name  # just the last key pressed
...
# keep track of which components have finished
InstructReminderComponents = [
    intructRemind_text1,
    instructRemind_text2,
    instructRemind_resp1,
]
...
while continueRoutine:
    ...
    if instructRemind_text2.status == NOT_STARTED and tThisFlip >= 0.0 - frameTolerance:
        ...
        win.timeOnFlip(
            instructRemind_text2, "tStartRefresh"
        )  # time at next scr refresh
    ...
    if instructRemind_resp1.status == NOT_STARTED and tThisFlip >= 0.0 - frameTolerance:
        ...
        win.timeOnFlip(
            instructRemind_resp1, "tStartRefresh"
        )  # time at next scr refresh
        ...
        win.callOnFlip(
            instructRemind_resp1.clearEvents, eventType="keyboard"
        )  # clear events on next screen flip
    if instructRemind_resp1.status == STARTED and not waitOnFlip:
        ...
        if len(_instructRemind_resp1_allKeys):
            instructRemind_resp1.keys = _instructRemind_resp1_allKeys[
                -1
            ].name  # just the last key pressed
...
while continueRoutine:
    ...
    if start_resp2.status == NOT_STARTED and tThisFlip >= 0.0 - frameTolerance:
        ...
        win.callOnFlip(
            start_resp2.clearEvents, eventType="keyboard"
        )  # clear events on next screen flip
    if start_resp2.status == STARTED and not waitOnFlip:
        ...
        if len(_start_resp2_allKeys):
            start_resp2.keys = _start_resp2_allKeys[
                -1
            ].name  # just the last key pressed
...
# set up handler to look after randomisation of conditions etc
Block2_loop = data.TrialHandler(
    nReps=1,
    method="sequential",
    extraInfo=expInfo,
    originPath=-1,
    trialList=data.importConditions(f"blocks/{subj}/Block2.csv"),
    seed=None,
    name="Block2_loop",
)
thisExp.addLoop(Block2_loop)  # add the loop to the experiment
thisBlock2_loop = Block2_loop.trialList[
    0
]  # so we can initialise stimuli with some values
...
for thisBlock2_loop in Block2_loop:
    ...
    _key_resp2_allKeys = []
    # keep track of which components have finished
    Block2Components = [image2, text2, key_resp2]
    ...
    while continueRoutine:
        ...
        if key_resp2.status == STARTED and not waitOnFlip:
            ...
            if len(_key_resp2_allKeys):
                key_resp2.keys = _key_resp2_allKeys[
                    -1
                ].name  # just the last key pressed
                key_resp2.rt = _key_resp2_allKeys[-1].rt
                # was this correct?
                if (key_resp2.keys == str(Block2Corr)) or (
                    key_resp2.keys == Block2Corr
                ):
                    key_resp2.corr = 1
                else:
                    key_resp2.corr = 0
    ...
    Block2_loop.addData("text2.started", text2.tStartRefresh)
    Block2_loop.addData("text2.stopped", text2.tStopRefresh)
    # check responses
...
while continueRoutine:
    ...
    if break_resp2.status == NOT_STARTED and tThisFlip >= 0.0 - frameTolerance:
        ...
        win.callOnFlip(
            break_resp2.clearEvents, eventType="keyboard"
        )  # clear events on next screen flip
    if break_resp2.status == STARTED and not waitOnFlip:
        ...
        if len(_break_resp2_allKeys):
            break_resp2.keys = _break_resp2_allKeys[
                -1
            ].name  # just the last key pressed
...
while continueRoutine:
    ...
    if start_resp3.status == NOT_STARTED and tThisFlip >= 0.0 - frameTolerance:
        ...
        win.callOnFlip(
            start_resp3.clearEvents, eventType="keyboard"
        )  # clear events on next screen flip
    if start_resp3.status == STARTED and not waitOnFlip:
        ...
        if len(_start_resp3_allKeys):
            start_resp3.keys = _start_resp3_allKeys[
                -1
            ].name  # just the last key pressed
...
# set up handler to look after randomisation of conditions etc
Block3_loop = data.TrialHandler(
    nReps=1,
    method="sequential",
    extraInfo=expInfo,
    originPath=-1,
    trialList=data.importConditions(f"blocks/{subj}/Block3.csv"),
    seed=None,
    name="Block3_loop",
)
thisExp.addLoop(Block3_loop)  # add the loop to the experiment
thisBlock3_loop = Block3_loop.trialList[
    0
]  # so we can initialise stimuli with some values
...
for thisBlock3_loop in Block3_loop:
    ...
    _key_resp3_allKeys = []
    # keep track of which components have finished
    Block3Components = [image3, text3, key_resp3]
    ...
    while continueRoutine:
        ...
        if key_resp3.status == NOT_STARTED and tThisFlip >= 0.0 - frameTolerance:
            ...
            win.callOnFlip(
                key_resp3.clearEvents, eventType="keyboard"
            )  # clear events on next screen flip
        if key_resp3.status == STARTED and not waitOnFlip:
            theseKeys = key_resp3.getKeys(keyList=["left", "right"], waitRelease=False)
            _key_resp3_allKeys.extend(theseKeys)
            if len(_key_resp3_allKeys):
                key_resp3.keys = _key_resp3_allKeys[
                    -1
                ].name  # just the last key pressed
                key_resp3.rt = _key_resp3_allKeys[-1].rt
                # was this correct?
                if (key_resp3.keys == str(Block3Corr)) or (
                    key_resp3.keys == Block3Corr
                ):
                    key_resp3.corr = 1
                else:
                    key_resp3.corr = 0
    ...
    Block3_loop.addData("text3.started", text3.tStartRefresh)
    Block3_loop.addData("text3.stopped", text3.tStopRefresh)
    # check responses
