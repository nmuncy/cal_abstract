...
subj = f'sub-{expInfo["participant"]}'
...
# make subj block files
def make_blocks(subj):
    ...
    block_dir = os.path.join(parent_dir, "data")
    ...
    for block in stim_dict:
        ...
        subj_dir = os.path.join(block_dir, subj)
...
Block1_loop = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions(f"data/{subj}/Block1.csv"),
...
Block2_loop = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions(f"data/{subj}/Block2.csv"),
...
Block3_loop = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions(f"data/{subj}/Block3csv"),
