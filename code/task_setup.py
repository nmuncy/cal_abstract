...
def main():
    ...
    for block in stim_dict:
        ...
        master_dict = {"Type": [], "Stim": [], "Corr": []}
        for key in block_dict.keys():
            for ind in range(0, block_length):
                master_dict["Type"].append(key)
                master_dict["Stim"].append(block_dict[key][1][ind])
                master_dict["Corr"].append(block_dict[key][0][ind])

        df = pd.DataFrame.from_dict(master_dict)
        df_rand = df.sample(frac=1).reset_index(drop=True)
        while df_rand.at[0, "Type"] == "Cond":
            df_rand = df.sample(frac=1).reset_index(drop=True)
